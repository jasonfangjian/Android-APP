...
{
	//原图
	Mat src = cv::imread("test/test8.jpg");	
	imshow("原图", src);
	waitKey(2500);
	//框架图
	Mat fuse = cv::imread("test/test8_fuse.png",0);
	imshow("边缘图", fuse);
	waitKey(2000);
	//转换为灰度图
	Mat gray= Mat::zeros(src.rows, src.cols, CV_8UC1);
	resize(fuse, gray, gray.size());
	imshow("尺寸调整之后", gray);            //图像缩放
	waitKey(2000);
	//图像增强
	vector<vector<Point>> contours;
	Mat Biny;
	threshold(gray, Biny, 30, 200, CV_THRESH_BINARY);
