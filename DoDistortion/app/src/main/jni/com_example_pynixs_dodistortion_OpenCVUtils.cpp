#include "com_example_pynixs_dodistortion_OpenCVUtils.h"
#include <iostream>
#include <string>
#include <sstream>
#include <array>
...
Mat cutImage(Mat &src, std::vector<cv::Point> cornors) {
	//放缩角点
	...
	srcVertex[0].x = cornors[0].x*widthRate;
	srcVertex[0].y = cornors[0].y*heightRate;
	srcVertex[1].x = cornors[1].x*widthRate;
	srcVertex[1].y = cornors[1].y*heightRate;
	srcVertex[2].x = cornors[3].x*widthRate;
	srcVertex[2].y = cornors[3].y*heightRate;
	srcVertex[3].x = cornors[2].x*widthRate;
	srcVertex[3].y = cornors[2].y*heightRate;

	dstVertex[0].x = cornors[0].x*widthRate;
	dstVertex[0].y = cornors[0].y*heightRate;
	dstVertex[1].x = cornors[1].x*widthRate;
	dstVertex[1].y = cornors[0].y*heightRate;
	dstVertex[2].x = cornors[0].x*widthRate;
	dstVertex[2].y = cornors[3].y*heightRate;
	dstVertex[3].x = cornors[1].x*widthRate;
	dstVertex[3].y = cornors[3].y*heightRate;
	//透视
...
JNIEXPORT jobject JNICALL Java_com_example_pynixs_dodistortion_OpenCVUtils_getDistortion
    ...
    //放缩灰度化
    Mat srcGray(mSrc.rows, mSrc.cols, CV_8UC1);
    resize(mSrc,srcGray,mFrame.size());
    Mat frameGray;
    cvtColor(mFrame, frameGray, CV_RGB2GRAY);
    //返回元组
    std::tuple<bool, std::vector<cv::Point>, std::vector<cv::Mat> > tup;
    //处理
    tup = ProcessEdgeImage(frameGray, srcGray, true);
    //拆分返回值
    bool jug;
